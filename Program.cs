...
async Task<string> GetPublicIp(IPAddress address) {
    ...
    return await http.GetStringAsync($"https://api{(address.AddressFamily == AddressFamily.InterNetwork ? "4" : "6")}.ipify.org/");
}

async Task<dynamic> GetRecordList(IPAddress address, string domain, string subDomain, string type) {
    using var http = GetHttpClient(address);
    var data = $"login_token={config.Id},{config.Token}&format=json&domain={domain}&sub_domain={subDomain}&record_type={type}";
    var resp = await http.PostAsync("https://dnsapi.cn/Record.List", new StringContent(data, Encoding.UTF8, "application/x-www-form-urlencoded"));
...
async Task<dynamic> UpdateRecord(IPAddress address, string domain, string recordId, string subDomain, string record) {
    using var http = GetHttpClient(address);
    var data = $"login_token={config.Id},{config.Token}&format=json&domain={domain}&sub_domain={subDomain}&record_id={recordId}&value={record}&record_line=默认";
    var resp = await http.PostAsync("https://dnsapi.cn/Record.Ddns", new StringContent(data, Encoding.UTF8, "application/x-www-form-urlencoded"));
    return JObject.Parse(await resp.Content.ReadAsStringAsync());
}
...
while (true) { 
    foreach (var record in config.Records) { 
        ...
        if (record.Types.Contains("A")) {
            ...
            if (record.Nic != -1) {
                var nic = allNetworkInterfaces[record.Nic];
                var v4 = nic.GetIPProperties().UnicastAddresses.FirstOrDefault(a => a.Address.AddressFamily == AddressFamily.InterNetwork);
                if (v4 == null) {
        ...
        if (record.Types.Contains("AAAA")) {
            ...
            if (record.Nic != -1) {
                var nic = allNetworkInterfaces[record.Nic];
                var v6 = nic.GetIPProperties().UnicastAddresses.FirstOrDefault(a => a.Address.AddressFamily == AddressFamily.InterNetworkV6);
                if (v6 == null) {
